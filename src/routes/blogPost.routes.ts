...
const blogPostRouter: express.Router = express.Router();

// ** Custom validation helper ** \\
const validationService = new ValidationHandler();

// ** Custom blog post route ** \\
const getRoute = (resource: string, controller: (req: Request, res: ApiResponse) => ControllerResponse) => {
    return (
        blogPostRouter.get(resource,
            verifyAccessToken,
            validationService.newBlogPostValidation("get-paginated-blog-posts"),
...
blogPostRouter.post("/posts",
    verifyAccessToken,
    validationService.newBlogPostValidation("create-blog-post"),
    expressValidationHandler,
    createBlogPost
);

// ** Route to like/unlike a blog post ** \\
blogPostRouter.patch("/posts/:postId",
    verifyAccessToken,
    validationService.newBlogPostValidation("postId"),
    expressValidationHandler,
    likeOrUnlikeBlogPost,
);

// ** Route to get a single blog post ** \\
blogPostRouter.get("/posts/:postId",
    verifyAccessToken,
    validationService.newBlogPostValidation("postId"),
    expressValidationHandler,
    getSingleBlogPost,
...
blogPostRouter.post(`/posts/:postId/view`,
    verifyAccessToken,
    validationService.newBlogPostValidation("postId"),
    expressValidationHandler,
    newBlogPostView,
...
blogPostRouter.patch("/posts/:postId/archive",
    verifyAccessToken,
    validationService.newBlogPostValidation("postId"),
    expressValidationHandler,
    archiveOrUnarchiveBlogPost
