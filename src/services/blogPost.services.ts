...
import { ApiResponse, ApiResponsePayload, ConfiguredRequest } from "../types/auth.types.js";
...
const getData = async (
    sortingOrPaginationData: { page?: number, searchQuery?: string },
    blogPostType: "all-posts" | "liked-posts" | "created-posts" | "archived-posts",
    userId: string
) => {
    try {
        ...
        if (!cachedPosts) {
            ...
            const blogPostsBeingSent: BlogPostSchema[] = (initialBlogPosts as unknown as InitialDbBlogPosts[]).map((post) => {
            ...
            const parsedCachedPosts: BlogPostSchema[] = parsedCacheData.data;
        ...
        const postType: string =
            blogPostType === "all-posts" ? "blog posts" :
...
const getPaginatedBlogPosts = async (
    req: Request,
    res: ApiResponse,
    blogPostType: "all-posts" | "liked-posts" | "created-posts" | "archived-posts"
    ...
    try {
        ...
        const sortingOrPaginationData = (req as ConfiguredRequest).data as { page?: number, searchQuery?: string };

        // ** This is needed to get the users identity IF they are requesting for their liked or created posts ** \\
        let user: { _id: mongoose.Types.ObjectId } | null = null;
